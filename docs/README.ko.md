...
### 모델 지원
- **다중 버전 모델 지원**: PP-OCRv4, PP-OCRv5 및 PP-OCRv6 모델을 지원하여 유연하게 선택 가능
...
이 라이브러리는 네 가지 PaddleOCR 모델 버전을 지원합니다:
...
### PP-OCRv6
- **최신 버전**: 차세대 OCR 솔루션으로 정확도가 향상되었습니다.
- **향상된 기능**:
  - 복잡한 레이아웃 및 밀집된 텍스트 감지 성능 개선.
  - 다국어 시나리오 인식 정확도 향상.
  - 정확도와 속도 모두 최적화.
- **모델 파일**:
  - 검출 모델: `PP-OCRv6_mobile_det.mnn`
  - 인식 모델: `PP-OCRv6_mobile_rec.mnn`
  - 문자셋: `ppocr_keys_v6.txt`

### 모델 성능 비교

| 특성 | PP-OCRv4 | PP-OCRv5 | PP-OCRv5 FP16 | PP-OCRv6 |
|---|---|---|---|---|
| 언어 지원 | 중국어, 영어 | 다국어 (기본 중/영/일, 11+ 전용 모델) | 다국어 (기본 중/영/일, 11+ 전용 모델) | 다국어 (향상) |
| 문자 유형 | 중국어, 영어 | 간체/번체, 영어, 일본어, 병음 | 간체/번체, 영어, 일본어, 병음 | 간체/번체, 영어, 일본어, 병음 |
| 손글씨 인식 | 기본 지원 | 대폭 강화 | 대폭 강화 | 대폭 강화 |
| 세로쓰기 | 기본 지원 | 최적화 | 최적화 | 최적화 |
| 희귀 문자 | 제한적 지원 | 인식 강화 | 인식 강화 | 인식 강화 |
| 복잡한 레이아웃 | 기본 지원 | 양호 | 양호 | 우수 |
| 추론 속도 (FPS) | 1.1 | 1.2 | 1.2 | 1.2 |
| 메모리 (피크) | 422.22MB | 388.41MB | 388.41MB | ~380MB |
| 모델 크기 | 표준 | 표준 | 절반 | 표준 |
| 권장 시나리오 | 일반 문서 | 복잡한 시나리오 및 다국어 | 고성능 및 다국어 | 최신·최고 정확도 |

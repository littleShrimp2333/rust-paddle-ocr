...
### モデルサポート
- **マルチバージョン対応**：PP-OCRv4、PP-OCRv5 および PP-OCRv6 モデルをサポートし、柔軟に選択可能。
...
本ライブラリは4つのPaddleOCRモデルバージョンをサポートしています：
...
### PP-OCRv6
- **最新版**：次世代OCRソリューション、精度がさらに向上。
- **強化された機能**：
  - 複雑なレイアウトと高密度テキストの検出を改善。
  - 多言語シナリオでの認識精度が向上。
  - 精度と速度の両方を最適化。
- **モデルファイル**：
  - 検出モデル：`PP-OCRv6_mobile_det.mnn`
  - 認識モデル：`PP-OCRv6_mobile_rec.mnn`
  - 文字セット：`ppocr_keys_v6.txt`

### モデル性能比較

| 特性 | PP-OCRv4 | PP-OCRv5 | PP-OCRv5 FP16 | PP-OCRv6 |
|---|---|---|---|---|
| 言語サポート | 中国語、英語 | 多言語（デフォルト中/英/日、11+専用モデル） | 多言語（デフォルト中/英/日、11+専用モデル） | 多言語（強化） |
| 文字タイプ | 中国語、英語 | 簡体字/繁体字、英語、日本語、ピンイン | 簡体字/繁体字、英語、日本語、ピンイン | 簡体字/繁体字、英語、日本語、ピンイン |
| 手書き認識 | 基本的 | 大幅に強化 | 大幅に強化 | 大幅に強化 |
| 縦書きテキスト | 基本的 | 最適化 | 最適化 | 最適化 |
| 希少文字 | 限定的 | 認識強化 | 認識強化 | 認識強化 |
| 複雑なレイアウト | 基本的 | 良好 | 良好 | 優秀 |
| 推論速度 (FPS) | 1.1 | 1.2 | 1.2 | 1.2 |
| メモリ (ピーク) | 422.22MB | 388.41MB | 388.41MB | ~380MB |
| モデルサイズ | 標準 | 標準 | 半減 | 標準 |
| 推奨シーン | 一般文書 | 複雑なシーン・多言語 | 高性能要求・多言語 | 最新・最高精度 |
